...
def fetch_hibob_employees(csv_path="hibob_employees.csv"):
    ...
    with open(csv_path, newline='', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if emp.get("email"):
                emp["email"] = emp["email"].lower().strip()
...
def build_report(employees, assembled_people, team_name_to_id, team_id_to_name):
    ...
    for emp in employees:
        ...
        if not person:
            no_match.append({
                ...
                "team": raw_team,
                "level": emp.get("level", ""),
                "reason": "Email not found in Assembled",
            })
            continue
        ...
        if target_team_id and target_team_id not in person["teams"]:
            ...
            team_changes.append({
                ...
                "hibob_team": raw_team,
                "current_teams": ", ".join(team_id_to_name.get(t, t) for t in person["teams"]) or "-",
                "action": f"WARNING: HiBob team '{raw_team}' not found in Assembled",
            })
    ...
    summary_data = [
        ["Metric", "Count"],
        ["Matched via exact email", len(matched)],
...
def main():
    ...
    print("  Matching on: exact email address")
    print("=============================================")
    ...
    print(f"  Nothing was changed in Assembled!")
    print(f"------------------------------------------------")
